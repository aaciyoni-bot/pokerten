...
GOD (5 לחיצות על הלוגו + קוד מנהל). במשחק החי קיימת בקרת `avPeek` נפרדת, הנועלת את הסיבוב שנצפה להימורים.
...
ענף AVIATOR מפעיל את `deploy-functions.yml` עבור קובצי המנוע בלבד.
הפריסה מוגבלת לפונקציות AVIATOR מפורשות, ללא `--force`. שינוי ב־`aviatorStream.js`
בלבד פורס רק את `avStream`; שינוי במנוע או בחישוב המחיר פורס את רכיבי AVIATOR
בסדר התואם ללקוחות קיימים. פריסת חוקים נפרדת ואינה חלק מעדכון תזמון.

ענפי הפוקר מסננים את רכיבי AVIATOR מפריסת הפונקציות שלהם. יש לשמור גם בהם
עותק עדכני של מקורות AVIATOR ולהחריג את `avStream`, בלי למזג ענף משחק ישן בשלמותו
ובלי לפרסם פונקציות פוקר כחלק מהעתקה זו.
...
- **קוד המנהל** (פותח GOD MODE, לוח המנהל והטענות צ'יפים בזמן שכניסת גוגל מושבתת):
  ...
  (`GOD_HASH`). אלו שני hashes שונים; הקודים עצמם אינם ברֵפו. אין לחשוף או להחליף אותם כחלק מתיקון תזמון.
- **הרשאות מנהל לפי מייל:** `aaci.yoni@gmail.com` (משמש לזיהוי בלבד).
- **Firebase:** פרויקט `pokerten`; Firestore ב־`eur3`. הלקוח החדש משתמש ב־`europe-west1`, ליד מסד הנתונים. השירותים הישנים ב־`us-central1` נשמרים לתאימות. הגרלת נקודת הקראש נעשית
...
53 בדיקות מקומיות: `node --test functions/aviator.test.js functions/aviatorStream.test.js aviator-src/live/timing.test.cjs aviator-ux.test.cjs`.
...
3. workflow ענף AVIATOR פורס רק את רכיבי AVIATOR המפורשים, כולל ערוץ `avStream` באירופה. הוא פורס קודם `avBet` המחייב מזהי פעולה ומסמן הימורים חדשים `protocol:2`, ואז את יתר הפונקציות. אין `--force` ואין פריסה של פונקציות פוקר.
...
לאחר פריסת רכיבי AVIATOR, ה־workflow מפעיל `node functions/test/aviatorLiveProbe.cjs --confirm-play-money-production`. הבדיקה משתמשת בכניסת אורח זמנית משלה ובאסימוני המשחק בלבד. היא בודקת משיכה ידנית, מחיר מדויק במאיות, זיכוי בצ׳יפים שלמים, ניסיונות חוזרים ללא חיוב או זיכוי כפולים ומשיכה אוטומטית ללא שליחת פקודת משיכה מהלקוח. היתרה וההימור נקראים ב־Firestore עם הרשאת אותו אורח. חשבון ההתחברות הזמני נמחק בסיום; רישומי המשחק נשמרים להתחשבנות.
