...
/* --- rule 4: bot mode — 'strong' by default, 'oracle' only when the table says so */
{
  // River: bot holds 10-3 (bottom pair) on A K 3 7 Q; the opponent holds 5-4
  // — a missed hand, nothing. The opponent shoves. From public information
  // bottom pair must fold (rule 3). Seeing the cards, it is a 100% call.
  // The ONLY thing that may separate the two is the table's botMode.
  const board = [card("A", "♣"), card("K", "♥"), card("3", "♠"), card("7", "♦"), card("Q", "♣")];
  const mine = [card("10", "♠"), card("3", "♦")];
  const air = [card("5", "♥"), card("4", "♠")];
  const serverCalls = (botMode) => {
    const S = {id: "t", settings: {blinds: 0.5, baseGameType: "NLH", maxPlayers: 6, ...(botMode ? {botMode} : {})},
      players: {me: {uid: "me", seatIndex: 0, status: "active", bet: 0, stack: 200, isBot: true},
        opp: {uid: "opp", seatIndex: 1, status: "active", bet: 200, stack: 0, isBot: false}},
      gameState: {phase: "river", board, pots: [{amount: 210, eligible: []}], highestBet: 200,
        minRaise: 1, currentGameType: "NLH", activeTurnUid: "me"},
      priv: {me: mine, opp: air}, table: {}, raw: {}, effects: [], now: 17e11};
    let calls = 0;
    for (let i = 0; i < 60; i++) if ((E.botAction(S, "me") || {}).action !== "fold") calls++;
    return calls;
  };
  const clientCalls = (botMode) => {
    const g = {phase: "river", board, pots: [{amount: 210, eligible: []}], highestBet: 200, minRaise: 1,
      currentGameType: "NLH", handBB: 1, activeTurnUid: "me"};
    const t = {settings: {blinds: 0.5, ...(botMode ? {botMode} : {})}, players: {
      me: {uid: "me", status: "active", bet: 0, stack: 200, cards: mine},
      opp: {uid: "opp", status: "active", bet: 200, stack: 0, cards: air}}};
    let calls = 0;
    for (let i = 0; i < 60; i++) {
      const mv = clientBrain(g, t, {uid: "me", status: "active", bet: 0, stack: 200, cards: mine}) || {type: "call"};
      if (mv.type !== "fold") calls++;
    }
    return calls;
  };
  check("server: default mode — bottom pair folds the shove, cards unseen", serverCalls(undefined) === 0,
    `called ${serverCalls(undefined)}/60 with no botMode`);
  check("server: 'strong' mode — same", serverCalls("strong") === 0, `called ${serverCalls("strong")}/60`);
  check("server: 'oracle' mode — sees the missed hand and calls", serverCalls("oracle") === 60,
    `called ${serverCalls("oracle")}/60 in oracle mode`);
  check("client: default mode — bottom pair folds the shove, cards unseen", clientCalls(undefined) === 0,
    `called ${clientCalls(undefined)}/60 with no botMode`);
  check("client: 'strong' mode — same", clientCalls("strong") === 0, `called ${clientCalls("strong")}/60`);
  check("client: 'oracle' mode — sees the missed hand and calls", clientCalls("oracle") === 60,
    `called ${clientCalls("oracle")}/60 in oracle mode`);
  // and the mirror image: oracle mode FOLDS a hand that public information
  // would happily play — top pair against a set
  const nuts = [card("7", "♠"), card("7", "♥")];
  const topPair = [card("A", "♠"), card("J", "♥")];
  const S2 = {id: "t", settings: {blinds: 0.5, baseGameType: "NLH", maxPlayers: 6, botMode: "oracle"},
    players: {me: {uid: "me", seatIndex: 0, status: "active", bet: 0, stack: 200, isBot: true},
      opp: {uid: "opp", seatIndex: 1, status: "active", bet: 200, stack: 0, isBot: false}},
    gameState: {phase: "river", board, pots: [{amount: 210, eligible: []}], highestBet: 200,
      minRaise: 1, currentGameType: "NLH", activeTurnUid: "me"},
    priv: {me: topPair, opp: nuts}, table: {}, raw: {}, effects: [], now: 17e11};
  let folds = 0;
  for (let i = 0; i < 60; i++) if ((E.botAction(S2, "me") || {}).action === "fold") folds++;
