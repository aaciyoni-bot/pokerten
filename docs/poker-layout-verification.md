...
- Opponents have tightly overlapping six-card backs; all-in and showdown faces open into a wider fan with readable rank and suit indices. The local player's wider fan keeps every rank and suit index visible.
...
- All 20 mandatory pre-deploy checks pass; service worker version is 260 (base was 258).
...
An authenticated live game was not available during this change. The post-deploy smoke checklist in PREDEPLOY.md still needs a connected test account: NLH action progression, Omaha deal, Spin winner payout, table creation, and leaving a table. The newly supplied club-entry screenshots are implemented as large square, uncropped logos and centred swipe cards on phones, with a three-column desktop grid. Carousel navigation was verified in the browser. Exact GG administration and settlement matching still awaits references for those screens.
